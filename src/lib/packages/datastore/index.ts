...
export default class Datastore extends HxbAbstract {
  ...
  async get(projectId: string): Promise<DatastoreRes> {
    ...
    // handle call graphql
    try {
      const res: DtDatastoreRes = await this.gqClient.request(GET_DATASTORES, { projectId });
  ...
  async getDetail(datastoreId: string): Promise<DatastoreSettingRes> {
    ...
    // handle call graphql
    try {
      const res: DtDatastoreSettingRes = await this.gqClient.request(GET_DATASTORE_DETAIL, { datastoreId });
  ...
  async create(payload: CreateDatastoreFromSeedReq): Promise<CreateDatastoreFromSeedRes> {
    ...
    // handle call graphql
    try {
      const res: DtCreateDatastoreFromSeed = await this.gqClient.request(CREATE_DATASTORE_FROM_TEMPLATE, payload);
  ...
  async validateDatastoreDisplayID(payload: IsExistsDSDisplayIDExcludeOwnReq): Promise<ExistsDSDisplayIDExcludeOwnRes> {
    ...
    // handle call graphql
    try {
      const resUpdate: DtValidateBeforeUpdateDsRes = await this.gqClient.request(VALIDATE_DS_DISPLAY_ID, payload);
  ...
  async updateDatastoreSetting(payload: DatastoreUpdateSetting): Promise<ModelRes> {
    ...
    // handle call graphql
    try {
      const resUpdate: DtUpdateDatastore = await this.gqClient.request(UPDATE_DATASTORE_SETTING, payload);
  ...
  async getFields(datastoreId: string, projectId: string): Promise<DatastoreGetFieldsRes> {
    ...
    // handle call graphql
    try {
      const res: DtDatastoreGetFieldsRes = await this.gqClient.request(DS_FIELDS, { datastoreId, projectId });
  ...
  async getField(fieldId: string, datastoreId: string): Promise<DsFieldSettingsRes> {
    ...
    // handle call graphql
    try {
      const res: DtDsFieldSettings = await this.gqClient.request(DS_FIELD_SETTING, { fieldId, datastoreId });
  ...
  async getActions(datastoreId: string): Promise<DsActionRes> {
    ...
    // handle call graphql
    try {
      const res: DtDsActions = await this.gqClient.request(DS_ACTIONS, { datastoreId });
  ...
  async getStatuses(datastoreId: string): Promise<DsStatusRes> {
    ...
    // handle call graphql
    try {
      const res: DtDsStatus = await this.gqClient.request(DS_STATUS, { datastoreId });
  ...
  async getAction(datastoreId: string, actionId: string): Promise<DsActionSettingRes> {
    ...
    // handle call graphql
    try {
      const res: DtDsActionSetting = await this.gqClient.request(DS_ACTION_SETTING, { actionId, datastoreId });
  ...
  async getAutoNumber(
    ...
    // handle call graphql
    try {
      const res: DtDatastoreFieldsAutoNum = await this.gqClient.request(DATASTORE_GET_FIELD_AUTO_NUMBER, { datastoreId, fieldId, projectId, params });
  ...
  async deleteDatastore(datastoreId: string): Promise<ModelRes> {
    ...
    // handle call graphql
    try {
      const resUpdate: DtDeleteDatastore = await this.gqClient.request(DELETE_DATASTORE, { datastoreId });
