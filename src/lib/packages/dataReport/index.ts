...
export default class DataReport extends HxbAbstract {
  ...
  async getReports(projectId: string): Promise<GetReportsRes> {
    ...
    // handle call graphql
    try {
      const res: DtGetReports = await this.gqClient.request(GET_REPORTS, { projectId });
  ...
  async getDataReport(projectId: string, reportId: string, reportDataPayload?: ReportDataPayload): Promise<ReportDataRes> {
    ...
    // handle call graphql
    try {
      const res: DtReportData = await this.gqClient.request(REPORT_DEFAULT, { projectId, reportId, reportDataPayload });
