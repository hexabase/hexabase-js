...
export default class Storage extends HxbAbstract {
  ...
  async getFile(id: string): Promise<GetDownloadFileRes> {
    ...
    // handle call graphql
    try {
      const res: DtGetDownloadFile = await this.request(GET_DOWNLOAD_FILE, { id });
   ...
   */
   async createFile(payload: ItemFileAttachmentPl): Promise<ItemFileAttachmentRes> {
    ...
    // handle call graphql
    try {
      const res: DtItemFileAttachment = await this.request(FILE_ATTACHMENT, { payload });
  ...
  async delete(fileId: string): Promise<ModelRes> {
    ...
    // handle call graphql
    try {
      const res: DtStorage = await this.request(DELETE_STORAGE, fileId);
